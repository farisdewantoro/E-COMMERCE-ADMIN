...
import {getAllCategoryBannerType} from '../../../../actions/categoryActions';
...
class CategoryBannerType extends Component {
    state = {
        openDelete: false,
        selected: [],
        expanded: null
        ...
        return (
            <div>
                <Grid container direction="column" spacing={16}>
                    ...
                    <Grid item md={12}>
                        <Card>
                            <CardHeader title="Banner Category Type"/>
                            <Divider/>
                            <CardContent>
                                <Grid container direction="column" spacing={16}>

                                    {categories
                                        .type instanceof Array && categories
                                        .type
                                        .map((c, i) => {
                                            return (
                                            <ExpansionPanel
                                                ...
                                                <ExpansionPanelDetails>
                                                        <Grid container direction="row" spacing={16}>
                                                            ...
                                                            <Grid item xs={12}>
                                                                <Card>
                                                                    ...
                                                                    <Divider />
                                                                    <CardContent>
                                                                        {categories.image_desktop.filter(id=>id.category_type_id === c.id).length === 0 && (
                                                                            <Typography
                                                                                style={{
                                                                                    textAlign: 'center',
                                                                                    fontWeight: "bold"
                                                                        ...
                                                                        {categories.image_desktop.filter(id => id.category_type_id === c.id).map(cid=>{
                                                                            return(
                                                                                <div key={cid.id}>
                                                            ...
                                                            <Grid item md={12}>
                                                                <Card>
                                                                    ...
                                                                    <Divider />
                                                                    <CardContent>
                                                                        {categories.image_mobile.filter(id => id.category_type_id === c.id).length === 0 && (
                                                                            <Typography
                                                                                style={{
                                                                                    textAlign: 'center',
                                                                                    fontWeight: "bold"
                                                                        ...
                                                                        {categories.image_mobile.filter(id => id.category_type_id === c.id).map(cid => {
                                                                            return (
                                                                                <div key={cid.id}>
                                                                                    <img src={cid.link} alt={cid.alt} style={{ maxWidth: "100%",maxHeight:"500px" }} />
                                                                                    <Typography>
                                                            ...
                                                            {categories.image_desktop_promo.filter(id => id.category_type_id === c.id).length > 0 && (
                                                            <Grid item xs={12}>
                                                                <Card>
                                                                    <CardHeader subheader='DESKTOP BANNER PROMO' />
                                                                    <Divider />
                                                                    <CardContent>
                                                                            {categories.image_desktop_promo.filter(id => id.category_type_id === c.id).map(cid => {
                                                                                return (
                                                                                    <div key={cid.id}>
                                                                                        <img src={cid.link} alt={cid.alt} style={{ maxWidth: "100%",maxHeight:"500px" }} />
                                                                                        <Typography>
                                                            ...
                                                            {categories.image_mobile_promo.filter(id => id.category_type_id === c.id).length > 0 && (
                                                            ...
                                                            <Grid item md={12}>
                                                                <Card>
                                                                    ...
                                                                    <Divider />
                                                                    <CardContent>
                                                                            {categories.image_mobile_promo.filter(id => id.category_type_id === c.id).map(cid => {
                                                                                return (
                                                                                    <div key={cid.id}>
                                                                                        <img src={cid.link} alt={cid.alt} style={{ maxWidth: "100%",maxHeight:"500px" }} />
                                                                                        <Typography>
                                                    ...
                                                    <ExpansionPanelActions>
                                                        ...
                                                        <Button size="small" color="primary" component={Link} to={`/category/banner-type/edit/${c.id}`}>
                                                            EDIT
                                                        </Button>
                                                    </ExpansionPanelActions>
                                         </ExpansionPanel> 
...
CategoryBannerType.propType = {
    classes: PropTypes.object.isRequired,
    categories: PropTypes.object.isRequired,
    notifications: PropTypes.object.isRequired,
    getAllCategoryBannerType: PropTypes.func.isRequired
}

const mapStateToProps = (state) => ({categories: state.categories, notifications: state.notifications})

export default compose(connect(mapStateToProps, { getAllCategoryBannerType }), withStyles(styles))(CategoryBannerType);
